...
{#if auth.session}
  ...
  <div class="flex items-center gap-3 px-4 pt-3 pb-3 border-b border-base-200 dark:border-base-800">
    <div class="relative">
      <button
      ...
      {#if showRecent}
        <div class="absolute left-0 top-full mt-1 w-56 rounded-lg border border-base-200 dark:border-base-800 bg-base-50 dark:bg-base-950 shadow-lg z-50 py-1">
          ...
          {:else}
            {#each recentAccounts as account (account.did)}
              <a
                ...
                class="block px-3 py-2 text-sm hover:bg-base-200 dark:hover:bg-base-800 transition-colors {account.did === did ? 'bg-accent-100 dark:bg-accent-900/30 text-accent-700 dark:text-accent-300' : ''}"
                onclick={() => (showRecent = false)}
              >
                <span class="font-medium">{account.label}</span>
                <span class="text-xs text-base-400 font-mono ml-2">{account.did.slice(0, 12)}…</span>
              </a>
            {/each}
          {/if}
        </div>
